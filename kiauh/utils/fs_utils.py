...
def create_symlink(source: Path, target: Path, sudo=False) -> None:
    try:
        ...
        Logger.print_error(_("Failed to create symlink: {}").format(e))
        raise
    ...
    for f in _files:
        try:
            ...
            if call(cmd, stderr=DEVNULL, stdout=DEVNULL) == 1:
                Logger.print_info(_("File '{}' does not exist. Skipped ...").format(f))
                continue
            cmd = ["sudo", "rm", "-rf", f.as_posix()]
            run(cmd, stderr=PIPE, check=True)
            Logger.print_ok(_("File '{}' was successfully removed!").format(f))
            _removed.append(f)
        except CalledProcessError as e:
            Logger.print_error(_("Error removing file '{}': {}").format(f,e))
...
def remove_file(file_path: Path, sudo=False) -> None:
    try:
        ...
        log = _("Cannot remove file {}: {e.stderr.decode()}").format(file_path)
...
def run_remove_routines(file: Path) -> bool:
    try:
        if not file.is_symlink() and not file.exists():
            Logger.print_info(_("File '{}' does not exist. Skipped ...").format(file))
            return False
        ...
        if file.is_dir():
            ...
            Logger.print_error(_("File '{}' is neither a file nor a directory!").format(file))
            return False
        Logger.print_ok(_("File '{}' was successfully removed!").format(file))
        return True
    except OSError as e:
        Logger.print_error(_("Unable to delete '{}':\n{}").format(file, e))
        try:
            ...
            Logger.print_error(_("Error deleting '{}' with sudo:\n{}").format(file, e))
            Logger.print_error(_("Remove this directory manually!"))
            return False
...
def create_folders(dirs: List[Path]) -> None:
    try:
        for _dir in dirs:
            ...
            Logger.print_ok(_("Created directory '{}'!").format(_dir))
    except OSError as e:
        Logger.print_error(_("Error creating directories: {}").format(e))
        raise
...
def get_data_dir(instance_type: type, suffix: str) -> Path:
    ...
    if suffix != "":
        ...
        return Path.home().joinpath(_("printer_{}_data").format(suffix))
